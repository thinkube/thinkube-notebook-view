...
- `load` puts the iframe in a tab only once the server is known to run (thinkube-control's status, or an unauthenticated probe without a token); otherwise the tab shows a message with *Start the notebook server*. The iframe never loads the Hub's page that starts a server.
- `resolveTarget` turns a path under the notebooks folder into an address using `thinkubeNotebookView.baseUrl`, or `https://notebooks.<DOMAIN_NAME>/user/<user>/<route>/thinkube/notebooks/`, where the route is `lab/tree/` (JupyterLab, default) or `notebooks/` (single-document page) by the `page` setting; `applyPage` rewrites a given address to the chosen page.
