...
- `src/sidebar.ts` — the *Thinkube Notebooks* activity bar container with one view, *Servers*: a row per node, the kernels of a running server beneath it, and rows for the Hub's default server and unattended runs; refreshed every 15 s while visible.
