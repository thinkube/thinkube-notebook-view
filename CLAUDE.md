...
- `src/extension.ts` — the tabs, the notebook editor, the listener, the commands:
  ...
  - Addresses are `https://notebooks.<domain>/user/<user>/<server>/<route>/thinkube/notebooks/<path>`: `<server>` is the node's name (the Hub's default server is not used); `<route>` is `notebooks/` (single-document page, default) or `lab/tree/` by the `page` setting. `NOTEBOOK_PAGE` parses them; `applyPage` rewrites one to the chosen page.
  ...
  - `frameHtml` loads the tab's address with `tk-theme=light|dark` from the IDE's color theme kind (`ideThemeFor`, `themedUrl`); thinkube-notebooks-theme applies it. A change of the IDE's color theme reloads every tab. Panels stay keyed by the address without the parameter.
...
- `src/sidebar.ts` — the *Servers* view of the *Thinkube Notebooks* activity bar container: a row per node, the kernels of a running server beneath it, and rows for unattended runs; refreshed every 15 s while visible.
